...
// Simple Sieve parser for importing scripts (basic implementation)
const parseSieveScript = (script) => {
  try {
    ...
    // Extract if/elsif blocks as rules (very basic implementation)
    const ruleRegex = /if\s+(.*?)\s*\{([\s\S]*?)\}/g;
    ...
    ruleMatches.forEach(match => {
      ...
      // Very basic condition parsing - this would need to be much more sophisticated in a real implementation
      ...
      if (actions.includes('fileinto')) {
        const mailboxMatch = actions.match(/fileinto\s+["']([^"']+)["']/);
        rule.actions.push({
          type: 'fileinto',
          parameters: {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4">
            ...
            <div className="space-y-3">
              {Object.entries(SCRIPT_TEMPLATES).map(([key, template]) => (
                <button
                  key={key}
                  onClick={() => {
                    // Directly call with the key to avoid any closure issues
                    const templateToLoad = SCRIPT_TEMPLATES[key];
                    
                    if (rules.length > 0) {
                      if (!window.confirm(`Load "${templateToLoad.name}"? This will replace your current script.`)) {
                        return;
                      }
                    }
                    
                    // Create a deep copy of the template rules with new IDs
                    const newRules = templateToLoad.rules.map(rule => ({
                      ...rule,
                      id: createId(),
                      conditions: rule.conditions.map(condition => ({...condition})),
                      actions: rule.actions.map(action => ({...action}))
                    ...
                    if (templateToLoad.extensions) {
                      setExtensions(prev => ({
                        ...prev,
                        ...templateToLoad.extensions
                      }));
                    }
                    
                    setShowTemplates(false);
                    showNotification(`Loaded "${templateToLoad.name}" template`, "success");
                  }}
                      ...
                      <div key={ext} className="flex items-center group">
                        ...
                        <div className="relative group">
                          <button 
                            ...
                            title={EXTENSION_DESCRIPTIONS[ext]}
                            aria-label={`Information about ${ext} extension`}
                          >
                            <Info className="h-4 w-4" />
                          </button>
                          <div className="absolute right-0 bottom-full mb-2 w-48 bg-gray-800 text-white text-xs rounded p-2 opacity-0 group-hover:opacity-100 transition pointer-events-none">
      ...
      {showCode && (
        <div className="bg-white rounded-lg shadow-md p-4">
          <div className="flex justify-between items-center mb-2">
            <h2 className="text-xl font-semibold">Generated Sieve Code</h2>
            <div className="flex space-x-2">
              <button 
                onClick={copyToClipboard}
